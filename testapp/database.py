...
from store.models import Key, KeyValue, Pincode, Product, Variation, Image, Tag, Category, Value, Discount, Order, OrderCancellation, OrderItem
import random
...
def UPDATE_DISCOUNT():
    d1 = Discount(code="10P", percent=10)
    d1.save()
    d2 = Discount(code="20P", percent=20)
    d2.save()
    d3 = Discount(code="30P", percent=30)
    d3.save()
    ...
    for product in products:
        for _ in range(random.randint(1, 10)):
            v = Variation(
                ...
                availability=True,
                info=INFO,
                unit=random.choice(['Kg', 'Lt', 'Dz']),
                discount=random.choice(d),
